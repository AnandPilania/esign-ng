...
class eNearDocExpireDocumentController extends eCBaseController
{
    ...
    {
        try {
            $result = $this->documentHandlerService->initDocumentList(DocumentHandler::DocNearExpire); //1: tai lieu sap het han
    ...
    public function searchDocumentList(Request $request)
    {
        try {
            ...
            $result = $this->documentHandlerService->searchDocumentList($searchData, $draw, $start, $limit, $sortQuery, DocumentHandler::DocNearExpire);
            return $this->sendResponse($result, '');
        } catch (Exception $e) {
