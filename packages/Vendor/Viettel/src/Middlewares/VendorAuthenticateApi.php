...
class VendorAuthenticateApi
{
    ...
    public function handle($request, Closure $next, ...$guards)
    {
        ...
        if (!$user) {
            return $this->baseApi->sendError("Bạn chưa đăng nhập hệ thống", ApiHttpStatus::NOT_ACCEPTABLE);
        }
        if ($user->revoked == 1) {
            return $this->baseApi->sendError("Tài khoản của bạn chưa được kích hoạt hoặc đã bị khoá.", ApiHttpStatus::NOT_ACCEPTABLE);
        }
        return $next($request);
    }
