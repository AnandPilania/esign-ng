...
(function () {
	'use strict';
	angular.module("app").controller("AppCtrl", ['$scope', '$parse', '$http', '$window', '$timeout', '$filter', AppCtrl]);
	function AppCtrl($scope, $parse, $http, $window, $timeout, $filter) {
		...
		$scope.lstAgencyStatus = [
			{ 'id': '-1', 'description': 'COMMON.STATUS' },
			{ 'id': '1', 'description': 'COMMON.ACTIVE' },
			{ 'id': '0', 'description': 'COMMON.PAUSE' },
		]

		$scope.lstServiceConfigType = [
			{ 'id': '1', 'description': 'CONFIG.SERVICE_CONFIG.TYPE_CONTRACT' },
			// { 'id': '2', 'description': 'CONFIG.SERVICE_CONFIG.TYPE_TIME' },
		]
		...
		$scope.avoidXSSRender = function(column, row, data){
            var dynamic_scope = 'render' + column + row;
            var model = $parse(dynamic_scope);
            model.assign($scope, data);
            return `<span>{{${dynamic_scope}}}</span>`;
        }
	}
})();
