...
(function () {
    ...
    function UserCtrl($scope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, QuanLyDangNhap) {
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            QuanLyDangNhap.search(params).then(function (response) {
                ...
                ctrl.lstUser.forEach((e) => {
                    if (e.agency_id != null){
                        for(let i=0; i<ctrl.lstAgency.length; i++){
                            if(e.agency_id == ctrl.lstAgency[i].id){
                                e.agency_name = ctrl.lstAgency[i].agency_name;
                                break;
                            }
                        }
                    }
        ...
        ctrl.openAddUserModal = function () {
            ...
            $uibModal.open({
                animation: true,
                templateUrl: 'admin/views/modal/addUpdateUser.html',
        ...
        ctrl.openEditUserModal = function (row) {
            ...
            $scope.editUser.agency_id = "" + $scope.editUser.agency_id;
        ...
        ctrl.onChangePass = function(row){
            ...
            $uibModal.open({
                animation: true,
                templateUrl: 'admin/views/modal/changePasswordUser.html',
                windowClass: "fade show modal-blur",
                size: 'lg modal-dialog-centered',
                backdrop: 'static',
                backdropClass: 'show',
    ...
    function ModalInstanceCtrl($scope, $uibModalInstance, $http, $window, $filter, QuanLyDangNhap) {
        $scope.onCreateUpdateUser = function (type) {
            ...
            if(user.role_id == "2" && user.agency_id == "-1"){
                errorMess += $filter('translate')('CONFIG.USER.ERR_EMPTY_USER_AGENCY_ID');
            }
