...
(function () {
    'use strict';
    angular.module("app", ['datatables', 'datatables.select', 'webcam']).controller("SignManageCommerceCtrl", ['$scope', '$compile', '$uibModal', '$http', '$state', '$window', '$timeout', 'DTOptionsBuilder', 'DTColumnBuilder', '$filter', 'KySoTaiLieuThuongMai', 'XuLyTaiLieuThuongMai', 'WebcamService', SignManageCommerceCtrl]);
    function SignManageCommerceCtrl($scope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, KySoTaiLieuThuongMai, XuLyTaiLieuThuongMai, WebcamService) {
        ...
        ctrl.init = function() {
            KySoTaiLieuThuongMai.init().then(function (response) {
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            KySoTaiLieuThuongMai.search(params).then(function (response) {
        ...
        ctrl.dtColumns = [
            ...
            DTColumnBuilder.newColumn('sent_date').withTitle($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MANAGE_SENT_DATE')),
            DTColumnBuilder.newColumn('code').withTitle($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MANAGE_CODE')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('code', meta.row, data);
            }),
            DTColumnBuilder.newColumn('name').withTitle($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MANAGE_NAME')).renderWith(function (data, type, full, meta){
            ...
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MANAGE_METHOD')).withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
                var sign_method = [];
                var html = '';
            ...
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MANAGE_MY_ORGANIZATION')).withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
                var html = '';
                data.myOrganisation.forEach(e => {
                    if (e.state == 2) {
            ...
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MANAGE_PARTNER')).withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
                var html = '';
                data.partner.forEach(e => {
                    if (e.state == 2) {
                        html += '<a class="btn btn-teal rounded-circle popover-item w-4 h-4" href="" data-toggle="tooltip" data-placement="top" title="Người duyệt: ' + e.name + ' - ' + e.email + '"><i class="fas fa-pen-alt"></i></a>';
            ...
            DTColumnBuilder.newColumn('creator_name').withTitle($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MANAGE_CREATOR')),
            DTColumnBuilder.newColumn(null).withTitle("").withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
        ...
        //     let document = ctrl.lstSignManage[row];
        //     const confirm = NotificationService.confirm($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_CONFIRM', {'documentName': document.name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
        //     confirm.then(function () {
        //         KySoTaiLieuThuongMai.signDocument({ docId: document.id, isLoading: true }).then(function (response) {
        ...
        function ModalInstanceCtrl($scope, $uibModalInstance, $http, $window, $filter, XuLyTaiLieuThuongMai, KySoTaiLieuThuongMai) {
        ...
        ctrl.confirmSign = function (document) {
            const confirm = NotificationService.confirm($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_DIGITAL_SIGN_CONFIRM', {'documentName': document.name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    KySoTaiLieuThuongMai.signDocument({ docId: document.id, isLoading: true }).then(function (response) {
                        if (response.data.success) {
                            NotificationService.success($filter('translate')(response.data.message), $filter('translate')("COMMON.NOTIFICATION.SUCCESS"));
        ...
        ctrl.confirmSignOtp = function (document) {
            const confirm = NotificationService.confirm($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_OTP_SIGN_CONFIRM', {'documentName': document.name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    XuLyTaiLieuThuongMai.sendOtp({docId: document.id, isLoading: true}).then(function (response) {
                        if (response.data.success) {
                            $scope.otpData = {
                                "otp": "",
                                "docId": document.id,
        ...
        ctrl.confirmSignKyc = function (document) {
            const confirm = NotificationService.confirm($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_EKYC_SIGN_CONFIRM', {'documentName': document.name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    $scope.signEkycData = {
                        currentSignStep: 1,
                        isCapture: true,
        ...
        ctrl.onSignMultiDocument = function () {
            ...
            for(let k = i+1; k<getLstSignMethod.length; k++){
                ...
                if(signMethod.length == 0){
                    NotificationService.error($filter('translate')("COMMERCE.SIGN_MANAGE.SIGN_METHOD_ERROR"));
                    return canSign = false;
                }
            }
            if (lstSignManage.length == 0) {
                NotificationService.error($filter('translate')('COMMERCE.SIGN_MANAGE.ERR_NEED_CHOOSE_SIGN_MANAGE'), $filter('translate')("COMMON.NOTIFICATION.ERROR"));
            }
        ...
        ctrl.confirmMultiSign = function (lstSignManage) {
            const confirm = NotificationService.confirm($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_DIGITAL_SIGN_CONFIRM', {'documentLength': lstSignManage.length}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    KySoTaiLieuThuongMai.signMulti({ lstDocId: lstSignManage, isLoading: true }).then(function (response) {
                        if (response.data.success) {
                            $scope.onSearchSignManage();
                            NotificationService.success($filter('translate')(response.data.message), $filter('translate')("COMMON.NOTIFICATION.SUCCESS"));
                        }
        ...
        ctrl.confirmMultiSignOtp = function () {
            NotificationService.error($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_MULTI_OTP_ERROR'));
        }

        ctrl.confirmMultiSignKyc = function (lstSignManage, cccd) {
            const confirm = NotificationService.confirm($filter('translate')('COMMERCE.SIGN_MANAGE.SIGN_EKYC_SIGN_CONFIRM', {'documentLength': lstSignManage.length}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    $scope.signEkycData = {
                        currentSignStep: 1,
                        isCapture: true,
        ...
        function ModalDocInstanceCtrl($scope, $uibModalInstance, $http, $window, $filter, XuLyTaiLieuThuongMai) {
