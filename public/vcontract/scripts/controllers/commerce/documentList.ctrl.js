...
(function () {
    'use strict';
    angular.module("app", ['datatables', 'datatables.select']).controller("DocumentListCommerceCtrl", ['$scope', '$compile', '$uibModal', '$http', '$state', '$window', '$timeout', 'DTOptionsBuilder', 'DTColumnBuilder', '$filter', 'DanhSachTaiLieuThuongMai', 'XuLyTaiLieuThuongMai', DocumentListCommerceCtrl]);
    function DocumentListCommerceCtrl($scope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, DanhSachTaiLieuThuongMai, XuLyTaiLieuThuongMai) {
        ...
        ctrl.init = function() {
            DanhSachTaiLieuThuongMai.init().then(function (response) {
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            DanhSachTaiLieuThuongMai.search(params).then(function (response) {
        ...
        ctrl.dtColumns = [
            ...
            DTColumnBuilder.newColumn('name').withTitle($filter('translate')('COMMERCE.DOCUMENT_LIST.DOCUMENT_LIST_NAME')).renderWith(function (data, type, full, meta){
            ...
            DTColumnBuilder.newColumn('expired_date').withTitle($filter('translate')('COMMERCE.DOCUMENT_LIST.DOCUMENT_LIST_EXPIRED_TIME')),
            DTColumnBuilder.newColumn('finished_date').withTitle($filter('translate')('COMMERCE.DOCUMENT_LIST.DOCUMENT_LIST_IS_COMPLETED')),
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('COMMERCE.DOCUMENT_LIST.DOCUMENT_LIST_DOC_STATE')).renderWith(function(data, type, full, meta){
        ...
        ctrl.openEditDocumentListModal = function (row) {
            ...
            if (document.document_state == 1) {
                $state.go("editCommerce", {docId: document.id});
            } else {
        ...
        ctrl.onDeleteMultiDocumentList = function () {
            ...
            if (lstDocumentList.length == 0) {
                NotificationService.error($filter('translate')('COMMERCE.DOCUMENT_LIST.ERR_NEED_CHOOSE_DOCUMENT_LIST'), $filter('translate')("COMMON.NOTIFICATION.ERROR"));
            } else {
                const confirm = NotificationService.confirm($filter('translate')('COMMERCE.DOCUMENT_LIST.DELETE_MULTI_CONFIRM', { 'documentListLength': lstDocumentList.length }), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    DanhSachTaiLieuThuongMai.deleteMulti({'lst': lstDocumentList, isLoading: true}).then(function (response) {
    ...
    function ModalInstanceCtrl($scope, $uibModalInstance, $http, $window, $filter, DanhSachTaiLieuThuongMai) {
