...
(function () {
    'use strict';
    angular.module("app", ['datatables', 'datatables.select','DocumentSrvc']).controller("ExpiredDocumentListCtrl", ['$scope', '$compile', '$uibModal', '$http', '$state', '$window', '$timeout', 'DTOptionsBuilder', 'DTColumnBuilder', '$filter', 'DanhSachTaiLieuHetHan', 'XuLyTaiLieuT',ExpiredDocumentListCtrl]);
    function ExpiredDocumentListCtrl($scope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, DanhSachTaiLieuHetHan, XuLyTaiLieuT) {
        ...
        //init
        ctrl.init = function() {
            DanhSachTaiLieuHetHan.init().then(function (response) {
        ...
        var titleHtml = '<input type="checkbox" class="form-check-input m-0 align-middle select-row" ng-model="expiredDocumentListCtrl.selectAll" ng-change="expiredDocumentListCtrl.toggleAll(expiredDocumentListCtrl.selectAll, expiredDocumentListCtrl.selected)">';
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            DanhSachTaiLieuHetHan.search(params).then(function (response) {
        ...
        ctrl.dtColumns = [
            ...
            DTColumnBuilder.newColumn(null).withTitle(titleHtml).notSortable().renderWith(function (data, type, full, meta) {
                ...
                return '<input type="checkbox" class="form-check-input m-0 align-middle select-row" ng-model="expiredDocumentListCtrl.selected[' + data.id + ']" ng-change="expiredDocumentListCtrl.toggleOne(expiredDocumentListCtrl.selected)"/>';
            }),
            DTColumnBuilder.newColumn('sent_date').withTitle(ctrl.searchDocumentList.parent_id == -1 ? $filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.DOCUMENT_LIST_SENT_DATE') : $filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.ADDENDUM_LIST_SENT_DATE')),
            DTColumnBuilder.newColumn('code').withTitle($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.DOCUMENT_LIST_CODE')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('code', meta.row, data);
            }),
            DTColumnBuilder.newColumn('name').withTitle($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.DOCUMENT_LIST_NAME')).renderWith(function (data, type, full, meta){
            ...
            DTColumnBuilder.newColumn('expired_date').withTitle($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.DOCUMENT_LIST_EXPIRED_TIME')),
            ...
            DTColumnBuilder.newColumn(null).withTitle("").withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
                ...
                return Utils.renderDataTableAction(meta, "expiredDocumentListCtrl.openEditDocumentListModal", false, false, false, false, false, "expiredDocumentListCtrl.onDownloadDocumentList", "expiredDocumentListCtrl.onHistoryDocumentList", false, false);
            }),
        ];
        ctrl.dtColumnsAddendum = [
            ...
            DTColumnBuilder.newColumn('sent_date').withTitle($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.ADDENDUM_LIST_SENT_DATE')),
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.ADDENDUM_MANAGE_PARENT_NAME')).notSortable().renderWith(function (data, type, full, meta) {
            ...
            DTColumnBuilder.newColumn('code').withTitle($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.ADDENDUM_LIST_CODE')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('code', meta.row, data);
            }),
            DTColumnBuilder.newColumn('name').withTitle($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.ADDENDUM_LIST_NAME')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('name', meta.row, data);
            }),
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('UTILITES.DOCUMENT_TYPE.DOCUMENT_STYLE.DEFAULT')).notSortable().renderWith(function (data, type, full, meta) {
                return $filter('translate')(Utils.getDocumentStyle(data.dc_style));
        ...
        $scope.changeDocumentGroup = function(){
            ...
            DanhSachTaiLieuHetHan.changeGroup(params).then(function (response) {
        ...
        ctrl.onDeleteMultiDocumentList = function () {
            ...
            if (lstDocumentList.length == 0) {
                ...
                const confirm = NotificationService.confirm($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.DELETE_MULTI_CONFIRM', { 'documentListLength': lstDocumentList.length }), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    DanhSachTaiLieuHetHan.deleteMulti({'lst': lstDocumentList}).then(function (response) {
        ...
        ctrl.openSendAgainMultiDocumentModal = function () {
            ...
            if (lstDocumentList.length == 0) {
                ...
                $uibModal.open({
                    animation: true,
                    templateUrl: 'vcontract/views/modal/sendAgainExpiredDocument.html',
    ...
    function ModalInstanceCtrl($scope, $uibModalInstance, $http, $window, $filter, DanhSachTaiLieuHetHan) {
        ...
        $scope.onSendAgainMultiDocumentList = function () {
            ...
            if (sendAgainDocument.expired_date == "") {
                errorMess += $filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.ERR_NEED_CHOOSE_EXPIRED_DATE');
            }

            if (errorMess == "") {
                const confirm = NotificationService.confirm($filter('translate')('DOCUMENT_HANDLE.EXPIRED_DOCUMENT_LIST.SEND_AGAIN_MULTI_CONFIRM', { 'documentListLength': sendAgainDocument.lst_document_list.length }), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    DanhSachTaiLieuHetHan.sendMulti(sendAgainDocument).then(function (response) {
                        if (response.data.success) {
                            $scope.onSearchDocumentList();
                            NotificationService.success($filter('translate')(response.data.message), $filter('translate')("COMMON.NOTIFICATION.SUCCESS"));
                        } else {
