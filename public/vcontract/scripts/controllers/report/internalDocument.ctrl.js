(function () {
    ...
    angular
        ...
        .controller("InternalDocumentReportCtrl", [
            "$scope", "$rootScope", "$compile", "$uibModal", "$http", "$state", "$window", "$timeout", "DTOptionsBuilder", "DTColumnBuilder", "$filter", "BaoCaoTaiLieuNoiBo", InternalDocumentReportCtrl,
        ]);
    function InternalDocumentReportCtrl($scope, $rootScope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, BaoCaoTaiLieuNoiBo) {
        var ctrl = this;
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            if (!ctrl.permission) {
                setTimeout(() => {
                    $scope.onSearchInternalDocument();
                }, 300);
                return;
            }
            ctrl.searchInternalDocument.startDate = Utils.parseDate(
                ctrl.searchInternalDocument.start_date
            );
            ctrl.searchInternalDocument.endDate = Utils.parseDateEnd(
                ctrl.searchInternalDocument.end_date
            );
            ...
            var params = {
                ...
                start: start,
                limit: length,
                searchData: ctrl.searchInternalDocument,
                isLoading: true,
            };
            ...
            BaoCaoTaiLieuNoiBo.search(params).then(
                function (response) {
                    ctrl.lstInternalDocument = response.data.data.data;
                    ctrl.lstInternalDocument.forEach((e) => {
                    ...
                    ctrl.lstInternalReport = response.data.data.data_report;
                    ctrl.lstInternalReport.forEach((e) => {
                    ...
                    $timeout(function () {
                        angular.element("#renderInternalChart").click();
                    }, 500);
        ...
        $scope.onSearchInternalDocument = function () {
            ctrl.dtInstance.rerender();
        };

        $scope.onViewReportDetail = function () {
            $("#reportInternalDetail").modal("show");
        };

        $scope.export = function () {
            ctrl.searchInternalDocument.startDate = Utils.parseDate(ctrl.searchInternalDocument.start_date);
            ctrl.searchInternalDocument.endDate = Utils.parseDate(ctrl.searchInternalDocument.end_date);
            var date_diff = Utils.dateDiffInDays(new Date(ctrl.searchInternalDocument.startDate),new Date(ctrl.searchInternalDocument.endDate));
            if (date_diff < 0) {
                ...
                formData.append("startDate",ctrl.searchInternalDocument.startDate);
                formData.append("endDate", ctrl.searchInternalDocument.endDate);
                formData.append("document_state", ctrl.searchInternalDocument.document_state);
                formData.append("document_type_id", ctrl.searchInternalDocument.document_type_id);
                formData.append("dc_style", ctrl.searchInternalDocument.dc_style);
                BaoCaoTaiLieuNoiBo.export(formData).then(
                    function (response) {
                        $(".loadingapp").addClass("hidden");
                        let fileName = "BaoCaoTaiLieuNoiBo_" + ctrl.searchInternalDocument.start_date + "_" + ctrl.searchInternalDocument.end_date + ".xlsx";
