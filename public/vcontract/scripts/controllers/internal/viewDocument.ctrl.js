(function () {
    'use strict';
    angular.module("app", ['datatables', 'datatables.select', 'InternalSrvc', 'webcam']).controller("ViewDocumentCtrl", ['$scope', '$rootScope', '$compile', '$uibModal', '$http', '$state', '$stateParams', '$window', '$timeout', 'DTOptionsBuilder', 'DTColumnBuilder', '$filter', 'XuLyTaiLieu', 'WebcamService', ViewDocumentCtrl]);
    ...
    function ViewDocumentCtrl($scope, $rootScope, $compile, $uibModal, $http, $state, $stateParams, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, XuLyTaiLieu, WebcamService) {
        ...
        $scope.initDocument = function (docId) {
            ...
            XuLyTaiLieu.initViewDoc({docId: docId, isLoading: true}).then(function (response) {
                ...
                $scope.getViewSignDocument(docId, ctrl.scale);
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            XuLyTaiLieu.searchAddendum(params).then(function (response) {
        ...
        ctrl.createAddendum = function () {
            $state.go("supplementInternal", {type: 0, parentId: ctrl.viewDoc.id , docId: ''});
        };

        ctrl.openEditAddendumListModal = function (row) {
            let document = ctrl.lstAddendumList[row];
            if (document.document_state == 1) {
                $state.go("editAddendumInternal", {parentId: ctrl.viewDoc.id , docId: document.id});
            } else {
        ...
        ctrl.onEditDenyDocument = function () {
            ...
            confirm.then(function () {
                 XuLyTaiLieu.editDocument({ docId: ctrl.viewDoc.id, isLoading: true }).then(function (response) {
                    if (response.data.success) {
                        if (ctrl.viewDoc.parent_id != -1) {
                            $state.go("editAddendumInternal", { parentId: ctrl.viewDoc.parent_id, docId: ctrl.viewDoc.id });
                        }
                        else {
                            $state.go("editInternal", { docId: ctrl.viewDoc.id });
                        }
                    }
                }, function (response) {
        ...
        $scope.onChangeScale = function () {
            ctrl.lstPosition = [];
            let id = $stateParams.docId;
            $scope.getViewSignDocument(id, ctrl.scale);
        }
        ...
        ctrl.onSaveLocation = function() {
            ...
            confirm.then(function () {
                XuLyTaiLieu.saveSignatureLocation({assigneeId: ctrl.viewDoc.cur.id, 'location': ctrl.lstPosition, isLoading: true}).then(function (response) {
                    if (response.data.success) {
                        ctrl.hasChangeSignature = false;
