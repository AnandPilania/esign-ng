...
(function () {
    'use strict';
    angular.module("app", ['datatables', 'datatables.select']).controller("ApprovalManageCtrl", ['$scope', '$compile', '$uibModal', '$http', '$state', '$window', '$timeout', 'DTOptionsBuilder', 'DTColumnBuilder', '$filter', 'PheDuyetTaiLieu', 'XuLyTaiLieu', ApprovalManageCtrl]);

    function ApprovalManageCtrl($scope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, PheDuyetTaiLieu, XuLyTaiLieu) {
        ...
        ctrl.init = function () {
            PheDuyetTaiLieu.init().then(function (response) {
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            PheDuyetTaiLieu.search(params).then(function (response) {
        ...
        ctrl.dtColumns = [
            ...
            DTColumnBuilder.newColumn('sent_date').withTitle($filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVAL_MANAGE_SENT_DATE')),
            DTColumnBuilder.newColumn('code').withTitle($filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVAL_MANAGE_CODE')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('code', meta.row, data);
            }),
            DTColumnBuilder.newColumn('name').withTitle($filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVAL_MANAGE_NAME')).renderWith(function (data, type, full, meta){
            ...
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVAL_MANAGE_MY_ORGANIZATION')).withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
                var html = '';
            ...
            DTColumnBuilder.newColumn(null).withTitle($filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVAL_MANAGE_PARTNER')).withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
                var html = '';
            ...
            DTColumnBuilder.newColumn('creator_name').withTitle($filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVAL_MANAGE_CREATOR')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('creator_name', meta.row, data);
            }),
            DTColumnBuilder.newColumn(null).withTitle("").withClass('text-center').notSortable().renderWith(function (data, type, full, meta) {
        ...
        ctrl.openApprovalManageModal = function (row) {
            ...
            if ($scope.approveDocument.email != $scope.loginUser.email) {
                NotificationService.error($filter('translate')('INTERNAL.APPROVAL_MANAGE.ERR_NOT_TURN_APPROVAL'), $filter('translate')("COMMON.NOTIFICATION.ERROR"));
                return;
            }
            $scope.approveParams = {
                isAgree: true,
        ...
        ctrl.openApproveMultiDocumentModal = function () {
            ...
            if ($scope.lstApprovalManageTmp.length == 0) {
                NotificationService.error($filter('translate')('INTERNAL.APPROVAL_MANAGE.ERR_NEED_CHOOSE_APPROVAL_MANAGE'), $filter('translate')("COMMON.NOTIFICATION.ERROR"));
            } else {
                $scope.approveType = 1;
                $scope.approveParams = {
                    isAgree: true,
    ...
    function ModalInstanceCtrl($scope, $uibModalInstance, $http, $window, $filter, PheDuyetTaiLieu) {
        $scope.onApproveDocumentConfirm = function (type) {
            var errorMess = "";
            if (!$scope.approveParams.isAgree && $scope.approveParams.content == '') {
                errorMess += $filter('translate')('INTERNAL.APPROVAL_MANAGE.ERR_EMPTY_APPROVE_DENIED_REASON');
            }
            if (errorMess == "") {
                if (type == 0) {
                    const confirm = NotificationService.confirm($scope.approveParams.isAgree ? $filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVE_CONFIRM', {'approveManage': $scope.approveDocument.name}) : $filter('translate')('INTERNAL.APPROVAL_MANAGE.DENY_CONFIRM', {'approveManage': $scope.approveDocument.name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                    confirm.then(function () {
                        PheDuyetTaiLieu.approve({
                    ...
                    const confirm = NotificationService.confirm($scope.approveParams.isAgree ? $filter('translate')('INTERNAL.APPROVAL_MANAGE.APPROVE_MULTI_CONFIRM', {'approvalManageLength': $scope.lstApprovalManageTmp.length}) : $filter('translate')('INTERNAL.APPROVAL_MANAGE.DENY_MULTI_CONFIRM', {'approvalManageLength': $scope.lstApprovalManageTmp.length}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                    confirm.then(function () {
                        PheDuyetTaiLieu.approveMulti({
