...
(function () {
    'use strict';
    angular.module("app", ['InternalSrvc', 'UtilitiesSrvc', 'ConfigService']).controller("CreateInternalDocumentCtrl", ['$scope', '$rootScope', '$compile', '$uibModal', '$http', '$state', '$stateParams', '$window', '$timeout', '$filter', 'DTOptionsBuilder', 'DTColumnBuilder', 'XuLyTaiLieu', 'NhanVien', 'KhachHangDoiTac', CreateInternalDocumentCtrl]);
    function CreateInternalDocumentCtrl($scope, $rootScope, $compile, $uibModal, $http, $state, $stateParams, $window, $timeout, $filter, DTOptionsBuilder, DTColumnBuilder, XuLyTaiLieu, NhanVien, KhachHangDoiTac) {
        ...
        ctrl.exitCompose = function() {
            ...
            confirm.then(function () {
                if($scope.isAddendum){
                    $state.go("index.internal.viewDocument", {docId: $stateParams.parentId});
                } else {
                    $state.go("index.internal.documentList");
                }
            }).catch(function (err) {
            });
        }
        ...
        ctrl.callInit = function (id) {
            XuLyTaiLieu.init({ docId: id, isLoading: true }).then(function (response) {
                ...
                if (!response.data.data.document) {
                    ...
                    // ctrl.newDoc = response.data.data.document;
                    ctrl.newDoc = {
                    ...
                    if (ctrl.newDoc.is_use_template) {
                        ...
                        XuLyTaiLieu.getDetailDocumentSampleById({ id: ctrl.newDoc.document_sample_id }).then(function (response) {
        ...
        ctrl.init = function () {
            ...
            if($stateParams.parentId){
                ...
                XuLyTaiLieu.getFatherDoc({ parent_id: $stateParams.parentId, isLoading: true }).then(function (response) {
        ...
        ctrl.onDocumentTypeChange = function () {
            if (ctrl.newDoc.document_type_id == "-1") {
                ...
                XuLyTaiLieu.getCodeById({ id: ctrl.newDoc.document_type_id, isLoading: true }).then(function (response) {
        ...
        ctrl.onDocumentSampleChange = function() {
            ...
            if (ctrl.newDoc.document_sample_id == "-1") {
                ...
                XuLyTaiLieu.getDetailDocumentSampleById({ id: ctrl.newDoc.document_sample_id, isLoading: true }).then(function (response) {
                    ctrl.sampleSignatures = response.data.data.signatures;
                    ctrl.documentSample = response.data.data.documentSample;
                    ctrl.sampleInfos = response.data.data.infos;
                    ...
                    XuLyTaiLieu.getCodeById({ id: ctrl.documentSample.document_type_id}).then(function (response) {
                        ctrl.newDoc.code = response.data.data;
                        if(ctrl.newDoc.code){
                            ctrl.splitCode(ctrl.newDoc.code);
                        }
        ...
        ctrl.goStep2 = function () {
            ...
            if (newDoc.is_use_template) {
                ...
                if (!errorMess) {
                    ...
                    XuLyTaiLieu.goChooseAssignee(newDoc).then(function (response) {
        ...
        ctrl.finishDrafting = function() {
            ...
            if (newDoc.is_use_template) {
                ...
                if (!errorMess) {
                    ...
                    XuLyTaiLieu.createDocumentFromTemplate(createdDoc).then(function (response) {
                ...
                XuLyTaiLieu.finishDrafting(data).then(function (response) {
                    NotificationService.success($filter('translate')(response.data.message), $filter('translate')("COMMON.NOTIFICATION.SUCCESS"));
                    if($scope.isAddendum){
                        $state.go("index.internal.viewDocument", {docId: $stateParams.parentId});
                    } else {
        ...
        ctrl.goStep3 = function () {
            ...
            if (newDoc.is_use_template) {
                ...
                XuLyTaiLieu.goStep3(data).then(function (response) {
                    if (response.data.success) {
        ...
        $scope.refreshDocumentCode = function(){
            XuLyTaiLieu.getCodeById({ id: ctrl.newDoc.document_type_id }).then(function (response) {
                ctrl.newDoc.code = response.data.data;
                ctrl.splitCode(ctrl.newDoc.code);
        ...
        $scope.removeFile = function(file, index){
            const confirm = NotificationService.confirm($filter('translate')('INTERNAL.COMMON.REMOVE_FILE_CONFIRM', {'fileName': file.name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
            confirm.then(function(response){
                XuLyTaiLieu.removeFile(file).then(function (response) {
                    if (response.data.success) {
        ...
        ctrl.selectAssignee = function (partner, index) {
            ...
            $uibModal.open({
                animation: true,
                templateUrl: "vcontract/views/internal/selectAssignee.html",
