...
(function () {
    'use strict';
    angular.module("app", ['datatables', 'datatables.select']).controller("SendSmsCtrl", ['$scope', '$compile', '$uibModal', '$http', '$state', '$window', '$timeout', 'DTOptionsBuilder', 'DTColumnBuilder', '$filter', 'QuanLyGuiSms', SendSmsCtrl]);
    function SendSmsCtrl($scope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, QuanLyGuiSms) {
        ...
        ctrl.init = function() {
            QuanLyGuiSms.init().then(function (response) {
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            QuanLyGuiSms.search(params).then(function (response) {
        ...
        ctrl.dtColumns = [
            ...
            DTColumnBuilder.newColumn('sent_date').withTitle($filter('translate')('INTERNAL.SEND_SMS.SEND_SMS_DOCUMENT_SENT_DATE')),
        ...
        ctrl.onSendSms = function (row) {
            ...
            const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_SMS.SEND_CONFIRM', {'sendSmsCode': sms.code, 'sendSmsReceiver': sms.receiver_name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
            confirm.then(function () {
                QuanLyGuiSms.send({convId: sms.id, isLoading: true}).then(function (response) {
        ...
        ctrl.onSendMultiSms = function () {
            ...
            if (lstSendSms.length == 0) {
                NotificationService.error($filter('translate')('INTERNAL.SEND_SMS.ERR_NEED_CHOOSE_SEND_SMS'), $filter('translate')("COMMON.NOTIFICATION.ERROR"));
            } else {
                const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_SMS.SEND_MULTI_CONFIRM', { 'sendSmsLength': lstSendSms.length }), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    QuanLyGuiSms.sendMulti({'lst': lstSendSms, isLoading: true}).then(function (response) {
                        if (response.data.success) {
                            $scope.onSearchSendSms();
                            NotificationService.success($filter('translate')(response.data.message), $filter('translate')("COMMON.NOTIFICATION.SUCCESS"));
                        } else {
        ...
        ctrl.onDeleteSendSms = function (row) {
            let sms = ctrl.lstSendSms[row];
            const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_SMS.DELETE_CONFIRM', {'sendSmsCode': sms.code}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
            confirm.then(function () {
                QuanLyGuiSms.delete({convId: sms.id, isLoading: true}).then(function (response) {
                    if (response.data.success) {
                        $scope.onSearchSendSms();
        ...
        ctrl.onDeleteMultiSendSms = function () {
            ...
            if (lstSendSms.length == 0) {
                ...
                const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_SMS.DELETE_MULTI_CONFIRM', { 'sendSmsLength': lstSendSms.length }), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    QuanLyGuiSms.deleteMulti({'lst': lstSendSms, isLoading: true}).then(function (response) {
                        if (response.data.success) {
                            $scope.onSearchSendSms();
