...
(function () {
    'use strict';
    angular.module("app", ['datatables', 'datatables.select']).controller("SendEmailCtrl", ['$scope', '$compile', '$uibModal', '$http', '$state', '$window', '$timeout', 'DTOptionsBuilder', 'DTColumnBuilder', '$filter', 'QuanLyGuiEmail', SendEmailCtrl]);
    function SendEmailCtrl($scope, $compile, $uibModal, $http, $state, $window, $timeout, DTOptionsBuilder, DTColumnBuilder, $filter, QuanLyGuiEmail) {
        ...
        ctrl.init = function() {
            QuanLyGuiEmail.init().then(function (response) {
        ...
        function getData(sSource, aoData, fnCallback, oSettings) {
            ...
            QuanLyGuiEmail.search(params).then(function (response) {
        ...
        ctrl.dtColumns = [
            ...
            DTColumnBuilder.newColumn('created_at').withTitle($filter('translate')('INTERNAL.SEND_EMAIL.SEND_EMAIL_SENDER_DATE')),
            DTColumnBuilder.newColumn('sender_name').withTitle($filter('translate')('INTERNAL.SEND_EMAIL.SEND_EMAIL_SENDER_NAME')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('sender_name', meta.row, data);
            }),
            DTColumnBuilder.newColumn('receiver_name').withTitle($filter('translate')('INTERNAL.SEND_EMAIL.SEND_EMAIL_RECEIVER_NAME')).renderWith(function (data, type, full, meta){
            ...
            DTColumnBuilder.newColumn('sent_date').withTitle($filter('translate')('INTERNAL.SEND_EMAIL.SEND_EMAIL_DOCUMENT_SENT_DATE')),
            DTColumnBuilder.newColumn('code').withTitle($filter('translate')('INTERNAL.SEND_EMAIL.SEND_EMAIL_DOCUMENT_CODE')).renderWith(function (data, type, full, meta){
                return $scope.avoidXSSRender('code', meta.row, data);
            }),
            DTColumnBuilder.newColumn('name').withTitle($filter('translate')('INTERNAL.SEND_EMAIL.SEND_EMAIL_DOCUMENT_NAME')).renderWith(function (data, type, full, meta){
        ...
        ctrl.onSendEmail = function (row) {
            ...
            const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_EMAIL.SEND_CONFIRM', {'sendEmailCode': email.code, 'sendEmailReceiver': email.receiver_name}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
            confirm.then(function () {
                QuanLyGuiEmail.send({convId: email.id, isLoading: true}).then(function (response) {
        ...
        ctrl.onSendMultiEmail = function () {
            ...
            if (lstSendEmail.length == 0) {
                NotificationService.error($filter('translate')('INTERNAL.SEND_EMAIL.ERR_NEED_CHOOSE_SEND_EMAIL'), $filter('translate')("COMMON.NOTIFICATION.ERROR"));
            } else {
                const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_EMAIL.SEND_MULTI_CONFIRM', { 'sendEmailLength': lstSendEmail.length }), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    QuanLyGuiEmail.sendMulti({'lst': lstSendEmail, isLoading: true}).then(function (response) {
                        if (response.data.success) {
                            $scope.onSearchSendEmail();
                            NotificationService.success($filter('translate')(response.data.message), $filter('translate')("COMMON.NOTIFICATION.SUCCESS"));
                        } else {
        ...
        ctrl.onDeleteSendEmail = function (row) {
            let email = ctrl.lstSendEmail[row];
            const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_EMAIL.DELETE_CONFIRM', {'sendEmailCode': email.code}), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
            confirm.then(function () {
                QuanLyGuiEmail.delete({convId: email.id, isLoading: true}).then(function (response) {
                    if (response.data.success) {
                        $scope.onSearchSendEmail();
        ...
        ctrl.onDeleteMultiSendEmail = function () {
            ...
            if (lstSendEmail.length == 0) {
                ...
                const confirm = NotificationService.confirm($filter('translate')('INTERNAL.SEND_EMAIL.DELETE_MULTI_CONFIRM', { 'sendEmailLength': lstSendEmail.length }), $filter('translate')("COMMON.NOTIFICATION.CONFIRM"), $filter('translate')("COMMON.NOTIFICATION.CANCEL"), $filter('translate')("COMMON.NOTIFICATION.OK"));
                confirm.then(function () {
                    QuanLyGuiEmail.deleteMulti({'lst': lstSendEmail, isLoading: true}).then(function (response) {
                        if (response.data.success) {
                            $scope.onSearchSendEmail();
